...
class SequenceNumberOperationsInterface {
 public:
  ...
  static T increment(T seq, T num = 1) const;
  static T decrement(T seq, T num = 1) const;

  static T wraparound(T seq) const;
   ...
   */
  static bool isNewer(T seq, T seq_compared) const;
   ...
   */
  static bool isInRange(T seq, T start, T end) const;
   ...
   */
  static T difference(T seq_smaller, T seq_larger) const;
};
...
class TimerInterface {
 public:
  ...
  virtual std::vector<TimerID> getActiveTimers() = 0;
};
...
class PacketMap : public PacketMapInterface<SequenceNumberType, PacketType> {
 public:
  ...
  std::shared_ptr<Packet> getPacket(SequenceNumber seq_num) override {
    ...
    return std::make_pair(true, packets_[seq_num]);
  }
...
class SRArqSender {
 public:
  ...
  SRArqSender(
      ...
      std::shared_ptr<CallbacksInterface> callbacks =
          std::make_shared<EmptyCallbacks>(),
      Mutex mutex = Mutex())
 ...
 private:
  ...
  void send(SequenceNumber seq_num, const std::vector<uint8_t>& data) {
    ...
    auto send_buffer = send_buffer_->getPacketsRef();
    send_buffer[seq_num] =
        std::make_shared<SendPacket>(PacketStatus::kSent, data, timer_id);
    // パケットを送信
  ...
  void onSendTimeout(SequenceNumber seq_num) {
    {  // ロックを取得
      ...
      send_buffer[seq_num]->status = PacketStatus::kTimeout;
    }  // ロックを解放

    callbacks_->onTimeout(seq_num);

    packet_transmitter_->transmitData(seq_num, send_buffer[seq_num]->data);
  }
 ...
 public:
  ...
  SRArqReceiver(
      ...
      std::shared_ptr<MissingPacketHandlerInterface<SequenceNumber>>
          missing_packet_handler = nullptr,
      Mutex mutex = Mutex())
  ...
  std::vector<uint8_t> getReceivedData(SequenceNumber seq_num) {
    {  // ロックを取得
      LockGuard guard(receive_buffer_mutex_);
      auto receive_buffer = receive_buffer_->getPacketsRef();
      return receive_buffer[seq_num]->data;
    }  // ロックを解放
  }
